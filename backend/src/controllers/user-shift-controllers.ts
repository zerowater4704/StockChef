...
export const requestShifts = async (req: Request, res: Response) => {
  try {
    ...
    let shiftDoc = await Shift.findOne({ userId, year, month });

    if (shiftDoc) {
      shifts.forEach((newShift: any) => {
        const existingShift = shiftDoc.shifts.find(
          (shift) => shift.date === newShift.date
        );
...
//　ユーザーが確定前のシフト確認
export const getPendingShifts = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { year, month } = req.query;

    const shiftDoc = await Shift.findOne({ userId, year, month });
    if (!shiftDoc) {
...
// ユーザーが確定後のシフト確認
export const getConfirmedShifts = async (req: Request, res: Response) => {
  try {
    const userId = req.user?.id;
    const { year, month } = req.query;
